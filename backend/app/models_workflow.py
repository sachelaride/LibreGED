import uuid
import json
from sqlalchemy import Column, String, Boolean, ForeignKey, DateTime, Integer, UniqueConstraint, Text
...
class WorkflowVersion(Base):
    __tablename__ = "ged_workflow_versions"
    __table_args__ = (
        UniqueConstraint("workflow_id", "version_number", name="uq_workflow_version_number"),
    )

    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    workflow_id = Column(String, ForeignKey("ged_workflows.id", ondelete="CASCADE"), nullable=False, index=True)
...
class WorkflowTask(Base):
    ...
    assignee_id = Column(String, ForeignKey("users.id", ondelete="RESTRICT"), nullable=True, index=True)
    assignee_group_id = Column(String, nullable=True, index=True)
    assignee_role = Column(String, nullable=True, index=True)
